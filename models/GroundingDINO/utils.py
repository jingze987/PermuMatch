...
from .backbone.swin_transformer import window_partition
...
def gen_encoder_output_proposals(
    ...
    for lvl, (H_, W_) in enumerate(spatial_shapes):
        ...
        if learnedwh is not None:
            ...
            wh = torch.ones_like(grid) * 0.05 * (2.0**lvl)  # wh is the anchor size for each point

        # scale = torch.cat([W_[None].unsqueeze(-1), H_[None].unsqueeze(-1)], 1).view(1, 1, 1, 2).repeat(N_, 1, 1, 1)
        # grid = (grid.unsqueeze(0).expand(N_, -1, -1, -1) + 0.5) / scale
        # wh = torch.ones_like(grid) / scale
        proposal = torch.cat((grid, wh), -1).view(N_, -1, 4)    # [x, y, w, h] for each proposal
...
def _set_lora_linear(model, rank):
    for name, module in model.named_children():
        if 'out_proj' in name or 'bbox_embed' in name:
            continue
        if isinstance(module, nn.Linear):
            setattr(model, name, lora.Linear(module.in_features, module.out_features, r=rank))
        else:
            _set_lora_linear(module, rank)
